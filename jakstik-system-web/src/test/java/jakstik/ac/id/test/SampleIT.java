...
public class SampleIT {
    ...
    public static Archive<?> getEarArchive() {
        ...
        EnterpriseArchive ear = ShrinkWrap.create(EnterpriseArchive.class, "jakstik-system-ear.ear");

        // Current directory is the root of the "jakstik-system-web" project. Go up one level
        // and enter the "jakstik-system-ejb" project.
        // The ejb jar is found in the "target" directory:
        File f = new File("../jakstik-system-ejb/target/jakstik-system-ejb.jar");
        JavaArchive ejbJar = ShrinkWrap.create(ZipImporter.class, "jakstik-system-ejb.jar").importFrom(f).as(JavaArchive.class);
        ear.addAsModule(ejbJar);

        // Now grab the web archive:
        f = new File("./target/jakstik-system-web.war");
        if (f.exists() == false) {
            throw new RuntimeException("File " + f.getAbsolutePath() + " does not exist.");
        }
        WebArchive war = ShrinkWrap.create(ZipImporter.class, "jakstik-system-web.war").importFrom(f).as(WebArchive.class);
        ear.addAsModule(war);
        ...
        war.addPackage("jakstik.ac.id.test");
